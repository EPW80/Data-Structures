...
template < typename T >
  class ExtendableVector {
    ...
    private: std::size_t _size = 0; // number of elements in the data structure
...
template < typename T >
  ExtendableVector < T > ::ExtendableVector(std::size_t capacity): _size(0), _capacity(capacity), _array(new T[capacity]()) {}
...
template < typename T >
  void ExtendableVector < T > ::push_back(const T & value) {
    insert(_size, value);
  } // delegate to insert() leveraging error checking

// Overloaded Array-Access Operator
template < typename T >
  T & ExtendableVector < T > ::operator[](std::size_t index) {
    return _array[index];
  } // Note: array bounds intentionally not checked

template < typename T >
  void ExtendableVector < T > ::set(std::size_t index,
    const T & value) {
    at(index) = value;
  } // delegate to at() leveraging error checking
...
template < typename T >
  std::size_t ExtendableVector < T > ::insert(std::size_t beforeIndex,
    const T & value) {
    if (_size >= _capacity)
      reserve(2 * _capacity); // If at max capacity, double the capacity
    if (beforeIndex > _size)
      beforeIndex = _size; // insert at the back
...
template < typename T >
  T * ExtendableVector < T > ::insert(T * beforePosition,
    const T & value) {
...
// Copy Constructor
template < typename T >
  ExtendableVector < T > ::ExtendableVector(const ExtendableVector < T > & other): _size(other._size), _capacity(other._capacity), _array(new T[other._capacity]()) {

...
void SLinkedList<Data_t>::reverse(Node *currentNode)
{
    ...
    if (currentNode == nullptr || currentNode->_next == nullptr)
        return;
...
typename SLinkedList<Data_t>::Iterator SLinkedList<Data_t>::removeAfter(const Iterator &position)
{
    ...
    if (position == _tail)
        return nullptr; // removing after the tail intentionally does nothing
    ...
    if (oldNode == _tail)
        _tail = nullptr; // remove the tail of the list

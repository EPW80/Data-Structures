...
{
    return _head->_next == _head; // can also use (_size == 0)  or  (_head->_prev == _head);
}
...
{
    if (empty())
        throw std::length_error("attempt to access data from an empty list");
...
{
    if (empty())
        throw std::length_error("attempt to remove from an empty list");
    if (current == _head)
        throw std::invalid_argument("Attempt to remove at an invalid location");
    ...
    delete current._node;                      // delete old node
    return returnNode;
}
...
{
    if (_node == nullptr)
        throw std::invalid_argument("Attempt to increment null Iterator");
    _node = _node->_next;
    return *this;
}
...
{
    if (_node == nullptr)
        throw std::invalid_argument("Attempt to decrement null Iterator");
    _node = _node->_prev;
    return *this;
}

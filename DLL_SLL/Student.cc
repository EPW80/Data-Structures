...
Student::Student(std::string name, unsigned nsem): _name(name), _numOfSemesters(nsem) // Example: constructor initialization list to initialize instance attributes
{}
...
bool operator == (const Student & lhs,
  const Student & rhs) {
  return lhs._name == rhs._name && lhs._numOfSemesters == rhs._numOfSemesters;
  // alternative:
